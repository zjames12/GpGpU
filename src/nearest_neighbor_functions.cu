...
__global__ void sort_distance_matrix(double* dist, int* indicies, int m) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i > n) {
        return;
    }
...
__global__ void create_nn_array(int* indicies, int NNarray, int m) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;
    if (i > n || j > m + 1){
        return;
...
int* nearest_neighbors(double* locs, int m, int n, int dim) {
    ...
    double *d_locs, *d_NNarray, *d_indicies, *d_dist;
    ...
    sort_distance_matrix<<<numBlocks2, threadsPerBlock2>>>(d_dist, d_indicies, m);
    cudaDeviceSynchronize();

    dim3 threadsPerBlock3(32, m + 1)
    create_nn_array<<<numBlocks2 , threadsPerBlock3>>>(d_indicies, d_NNarray, m);
